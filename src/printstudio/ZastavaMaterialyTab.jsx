...
export default function ZastavaMaterialyTab({ supabase }) {
  ...
  const nacitaj = async () => {
    setIsLoading(true);
    const [{ data: zm }, { data: sm }] = await Promise.all([
      supabase.from('zastava_materialy').select('*').order('poradie').order('id'),
      // Len materialy predavane na bezny meter (vlajkovina/rolkovy textil) — ostatne (ks, kg...) sa sem nehodia.
  ...
  // Prepojenie na sklad. material — hned dopocita a ulozi aj naklad_m2, ak sa da (ma vyplnenu sirku).

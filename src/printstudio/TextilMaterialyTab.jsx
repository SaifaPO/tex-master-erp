...
export default function TextilMaterialyTab({ supabase }) {
  ...
  const nacitaj = async () => {
    ...
    const [{ data: tm }, { data: sm }] = await Promise.all([
      supabase.from('textil_materialy').select('*').order('poradie').order('id'),
  ...
  const pridaj = async () => {
    const { data, error } = await supabase.from('textil_materialy').insert({ kod: `material_${Date.now()}`, nazov: 'Nová látka', naklad_m2: 0, technologia: 'obe', poradie: materialy.length }).select().single();
    if (!error && data) setMaterialy(m => [...m, data]);
  };
  const uprav = async (id, patch) => {
    setMaterialy(m => m.map(x => x.id === id ? { ...x, ...patch } : x));
    await supabase.from('textil_materialy').update(patch).eq('id', id);
  };
  const zmaz = async (id) => {
    if (!window.confirm('Zmazať túto látku?')) return;
    setMaterialy(m => m.filter(x => x.id !== id));
    await supabase.from('textil_materialy').delete().eq('id', id);
  };
    ...
    <div className="bg-slate-900/60 rounded-2xl border border-slate-800 p-5 space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Zákazník si okrem "vlastný materiál" (súčasný stav, len cena tlačového procesu) môže vybrať aj
          jednu z týchto látok — cena látky sa dopočíta jednotným maržovým vzorcom (Cenotvorba) a pripočíta
          ...
          return (
            <div key={m.id} className="bg-slate-950 border border-slate-800 rounded-xl p-3 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <select value={m.technologia} onChange={(e) => uprav(m.id, { technologia: e.target.value })} className="px-2 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white shrink-0">
              ...
              <input type="text" value={m.specifikacie || ''} onChange={(e) => uprav(m.id, { specifikacie: e.target.value })} placeholder="Technické špecifikácie — napr. 100% polyester, 150 g/m² (zobrazí sa zákazníkovi)" className="w-full px-2 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-300" />
            </div>
          );
        })}

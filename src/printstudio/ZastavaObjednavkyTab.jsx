...
export default function ZastavaObjednavkyTab({ supabase }) {
  ...
  const nacitaj = async () => {
    setIsLoading(true);
    let q = supabase.from('zastava_objednavky').select('*').order('created_at', { ascending: false });
  ...
  const zmenStav = async (o, status) => {
    ...
    await supabase.from('zastava_objednavky').update(patch).eq('id', o.id);
    nacitaj();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2"><ClipboardList className="text-indigo-400 h-5 w-5" /> Objednávky zástav</h2>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {objednavky.map(o => {
            ...
            const hardware = [];
            if (Array.isArray(o.tunely) && o.tunely.length) hardware.push(`${o.tunely.length}× tunel`);
            if (Array.isArray(o.ocka) && o.ocka.length) hardware.push(`${o.ocka.reduce((s, g) => s + (g.side === 'all' ? g.count * 4 : g.side === 'corners' ? 4 : g.count), 0)} ok`);
            if (Array.isArray(o.karabinky) && o.karabinky.length) hardware.push(`${o.karabinky.reduce((s, c) => s + (c.side === 'all' ? c.count * 4 : c.count), 0)} karabín`);
            if (o.popruhy && Object.values(o.popruhy).some(Boolean)) hardware.push('popruh');
            return (
              <div key={o.id} className="bg-slate-900 rounded-2xl border border-slate-800 p-4 flex flex-col sm:flex-row gap-4">
                {o.nahlad_url && (
                  <img src={o.nahlad_url} alt="Náhľad zástavy" className="w-24 h-24 object-cover rounded-lg border border-slate-800 shrink-0" />
                )}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-white font-semibold">
                    {o.sirka_cm}×{o.vyska_cm} cm · {o.material_kod} · {o.pocet_ks} ks {o.expresne && <span className="text-amber-400">· expres</span>}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    Okraje: {o.vyhotovenie === 'laser' ? 'Laser orez' : 'Obšité'}{hardware.length > 0 && <> · {hardware.join(' · ')}</>}
                    {o.statna_vlajka && <> · Štátna vlajka: {o.statna_vlajka}</>}

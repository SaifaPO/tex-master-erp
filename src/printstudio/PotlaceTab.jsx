...
// Navrh minimalnej ceny ukonu = VC pri NAJLACNEJSOM realnom pripade (drobny/nulovy motiv, pri
// sitotlaci/vysivke aj velky odber - viz vcXxxPodlaha vyssie) × najnizsia marza z marzovej krivky.
// Len navrh (tlacidlo "Pouzit"), pole ostava vzdy rucne editovatelne.
function NavrhMinCeny({ vc, config, onPouzit }) {
  ...
  return (
    <div className="mt-1.5 flex items-center flex-wrap gap-x-2 gap-y-1 text-[11px] text-slate-500">
      <span>Najlacnejšie, čo tento úkon reálne môže vyjsť (drobný motív, veľký odber, marža {floor.toFixed(0)}%): <strong className="text-slate-300">{navrh.toFixed(2)}€</strong>.</span>
...
export default function PotlaceTab({ supabase }) {
  ...
  // "Podlahova" VC pre navrh min. ceny ukonu — NAJLACNEJSI realny pripad (drobny/nulovy motiv,
  // pri sitotlaci/vysivke aj velky odber), nie hodnoty z "Referencnej objednavky" hore (tie su

...
export default function DtfMetraz({ supabase, onSpat }) {
  ...
  const odoslatObjednavku = async () => {
    ...
    try {
      ...
      let suborCesta = null;
      if (rawFile) {
        const cesta = `dtf/${objednavkaIdPredbezne}/${rawFile.name}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h3 className="text-sm font-bold text-slate-900 mb-1">Prehľad množstevných zliav (šírka 56 cm)</h3>
        <p className="text-[11px] text-slate-400 mb-3">Ceny v tabuľke sú bez DPH.</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 font-semibold">
                <th className="p-2.5">Metráž</th><th className="p-2.5">Cena €/bm (bez DPH)</th><th className="p-2.5">Zľava</th>
              </tr>
            </thead>
